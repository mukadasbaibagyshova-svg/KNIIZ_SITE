...
$page_description = t("meta_desc_news");
$page_keywords = t("meta_keys_news");
...
<main class="py-5">
    <div class="container">
        ...
        <div class="mb-5 text-center">
            ...
            <p class="section-subtitle-premium text-muted mx-auto"><?php echo t(
                "news_intro",
            ); ?></p>
        </div>
        ...
        if ($all_news):
            foreach ($all_news as $news):
                ...
                $category = isset($news["category"])
                    ? htmlspecialchars($news["category"])
                    : t("news_category_default");
            ...
            <div class="news-card" data-news='<?= $news_json ?>' data-upload-dir="<?= $upload_dir ?>">
                <div class="news-card-img">
                    <img src="<?= $img ?>" alt="<?php echo htmlspecialchars(
    t("photo_alt"),
); ?>">
                </div>
                <div class="news-card-body">
                    <div class="news-card-category"><?= $category ?></div>
                    <div class="news-card-title"><?= htmlspecialchars(
                        $title,
                    ) ?></div>
                    <div class="news-card-desc"><?= htmlspecialchars(
                        $desc,
                    ) ?></div>
                </div>
            </div>
